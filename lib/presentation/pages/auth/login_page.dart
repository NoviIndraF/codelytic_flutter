...
class LoginPage extends StatelessWidget {
      ...
      listener: (context, state) {
        if (state is AuthGetRoom){
          if(state.result.data.isEmpty){
            ...
            Navigator.popAndPushNamed(context, AppRoute.loginClass, arguments: {"data": state.result.data});
          }
        }
        ...
        if (state is AuthRegistered){
          ShowWidget.showSnackBar(context, "Registrasi Berhasil, Silahkan login terlebih dahulu", "success");
        }
  ...
  StatelessWidget loginButton(BuildContext context, GlobalKey<FormState> formGlobalKey, ) {
    return Container(
      height: 50,
      width: Dimens.widthMax(context),
      margin: const EdgeInsets.only(top: 30),
      child: TextButton(
          onPressed: () {
            if(_validate(_formLogin)){
              final request = LoginRequest(email: emailController.text, password: passwordController.text);
